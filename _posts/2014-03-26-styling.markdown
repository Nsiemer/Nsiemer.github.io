...
Just realizing that it would probably be nice to have the content provider's name in the top left of the citation div. Later. The citation bubble is kind of a pain right now, having to give the pronoun an `<h4>` tag, then typing out each link and adding a `target="_blank"`. I want to come up with some meta integrating thing where I can just put all the links into an array.
<hr>
